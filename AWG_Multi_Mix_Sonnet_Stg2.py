...
# Stage-by-stage implementation with terminal menu control
...
MULTIMETER_MODE_BUTTON_PIN = 16    # Multimeter mode cycling (optional - menu controlled)
BUZZER_PIN = 15         # Buzzer control (moved from 17)
RESISTANCE_ADC_PIN = 27 # Resistance measurement (moved from 26 to 27)

# === SYSTEM CONTROL ===
SYSTEM_MODE_BUTTON_PIN = 22  # Switch between AWG/Multimeter/Both modes (optional - menu controlled)
...
CONTINUITY_THRESHOLD_OHMS = 1200.0    # Continuity threshold
...
class PicoTestStation:
    ...
    # ========================================================================
    # STAGE INITIALIZATION (Same as before)
    # ========================================================================
    ...
    def init_multimeter(self):
        ...
        try:
            ...
            # Optional physical buttons (can be overridden by menu)
            if hasattr(self, 'multimeter_mode_button'):
    ...
    def init_system_controls(self):
        ...
        try:
            # Optional physical button (can be overridden by menu)
            if hasattr(self, 'system_mode_button'):
    ...
    # ========================================================================
    # MULTIMETER FUNCTIONS (Same as before)
    # ========================================================================
    ...
    def handle_menu_command(self, command):
        ...
        elif command == 's' and self.stage >= 4:
        ...
        # AWG commands
            ...
            try:
                ...
                frequency = max(MIN_FREQ, min(MAX_FREQ, frequency))
                self.set_awg_output(frequency, self.current_waveform)
                print(f"Frequency set to: {self.format_frequency(frequency)}")
            except ValueError:
            ...
            try:
                ...
                if 0 <= choice < len(waveforms):
                    ...
                    self.set_awg_output(self.current_frequency, waveform)
    ...
    def show_current_readings(self):
        ...
        if self.stage >= 2:
            try:
                ...
                resistance = data['resistance_ohms']
                if resistance < 1000:
                    print(f"Resistance: {resistance:.1f} Ohm")
                elif resistance < OPEN_CIRCUIT_THRESHOLD_OHMS:
                    print(f"Resistance: {resistance/1000:.1f} kOhm")
                else:
        ...
        if self.stage >= 3:
            try:
                awg_data = self.read_awg_controls()
                print(f"AWG Frequency: {self.format_frequency(awg_data['frequency'])}")
                print(f"AWG Waveform: {awg_data['waveform']}")
    ...
    # ========================================================================
    # MAIN OPERATION MODES (Same as before)
    # ========================================================================
...
def test_stage_3_menu():
    """Stage 3: Add AWG functionality with menu control"""
    print("\n" + "="*50)
    print("STAGE 3 TEST: AWG Integration (Menu Control)")
    print("="*50)
    print("This stage adds:")
    ...
    print("\nExpected: Menu-controlled multimeter and AWG")
    print("Commands: s=system mode, a=awg toggle, f=frequency, w=waveform")
    print("="*50)
...
if __name__ == "__main__":
    ...
    print("3. Multimeter + AWG (with menu)")
    ...
    test_stage_3_menu()  # Add AWG functionality
